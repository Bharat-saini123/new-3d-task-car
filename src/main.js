...
const thumbRenderer = new THREE.WebGLRenderer({
  canvas: document.querySelector("#thumb-canvas"),
  antialias: true,
  alpha: true,
});
...
function resize() {
  ...
  thumbRenderer.setSize(rect.width, rect.height, false);
  thumbCamera.aspect = rect.width / rect.height;
...
function getDisplayScale(index) {
  return [1, 3, 5].includes(index) ? 0.6 : index >= 7 ? 0.5 : 1;
}
function setStage(index, animate = false) {
  ...
  model.scale.multiplyScalar(Math.min(1, 3.2 / maxSize));
  const fittedBounds = new THREE.Box3().setFromObject(model);
  const fittedCenter = fittedBounds.getCenter(new THREE.Vector3());
  model.position.x -= fittedCenter.x;
  model.position.y -= fittedBounds.min.y;
  model.position.z -= fittedCenter.z;
  if (!animate || !model.animations?.length) {
    animation = null;
    return;
  }
  ...
  animation = {
    ...
    duration: model.animations[0].duration / ASSEMBLY_ANIMATION_SPEED,
  };
}
...
function updateUi() {
  ...
  document.querySelector(".nm").textContent =
    step === MODEL_COUNT - 1
      ...
      : `Assembly ${String(step + 1).padStart(2, "0")}`;
  ...
  document
    ...
    .forEach((tick, index) => tick.classList.toggle("done", index <= step));
...
for (let index = 0; index < MODEL_COUNT; index++) {
  ...
  tick.className = "tick";
  tick.style.left = `${(index / (MODEL_COUNT - 1)) * 100}%`;
...
document.querySelector("#btn-rotate").onclick = (event) => {
  autoRotate = !autoRotate;
  event.currentTarget.classList.toggle("active", autoRotate);
};
document.querySelector("#btn-view").onclick = (event) => {
  viewMode = viewMode === "iso" ? "top" : "iso";
  cameraTarget = (viewMode === "iso" ? isoPosition : topPosition).clone();
  event.currentTarget.classList.toggle("active", viewMode === "top");
};
...
canvas.onpointermove = (event) => {
  if (!dragging) return;
  userRotY += (event.clientX - lastPointer.x) * 0.008;
  userRotX = Math.max(
    -0.9,
    Math.min(0.9, userRotX + (event.clientY - lastPointer.y) * 0.006),
  );
  lastPointer = { x: event.clientX, y: event.clientY };
};
canvas.onpointerup = () => {
...
function render() {
  ...
  if (animation) {
    const elapsed = Math.min(
      animation.duration,
      (performance.now() - animation.start) / 1000,
    );
...
async function loadAssembly() {
  ...
  const rawModels = await Promise.all(
    Array.from({ length: MODEL_COUNT }, (_, index) =>
      loader.loadAsync(
        `${MODEL_BASE}shot${String(index + 1).padStart(2, "0")}.fbx`,
      ),
    ),
  );
  ...
  rawModels.forEach((model) => {
    ...
    largestDimension = Math.max(largestDimension, size.x, size.y, size.z);
  });
  ...
  const allBounds = new THREE.Box3().setFromObject(
    new THREE.Group().add(...models.map((model) => model.clone(true))),
  );
  lookAt = allBounds.getCenter(new THREE.Vector3());
  const maxDimension = Math.max(
    allBounds.getSize(new THREE.Vector3()).x,
    allBounds.getSize(new THREE.Vector3()).y,
    allBounds.getSize(new THREE.Vector3()).z,
  );
  isoPosition = lookAt
    .clone()
    .add(
      new THREE.Vector3(1.15, 0.42, 1.25)
        .normalize()
        .multiplyScalar(maxDimension * 1.8),
    );
  topPosition = lookAt
    .clone()
    .add(
      new THREE.Vector3(0.05, 1.5, 0.55)
        .normalize()
        .multiplyScalar(maxDimension * 2.15),
    );
  cameraTarget = isoPosition.clone();
...
loadAssembly().catch((error) => {
  document.querySelector(".loading").textContent = "Unable to load FBX assets";
  console.error(error);
});
render();
